if [ -z "${CORE_GIT}" ]; then
    >&2 echo "ERROR: environment variable \`CORE_GIT\` is not set. This enviroment variable should contain the path to a OhioCraft-patched minetest source tree."
    exit 1
fi
...
get_sources () {
	...
	if ! command -v rsync > /dev/null 2>&1; then
		>&2 echo "ERROR: \`rsync\` is not available in \$PATH"
		exit 1
	fi
    local coregit=$(echo "$CORE_GIT" | sed 's:/*$::')
    if [ -d $coregit/lib/irrlichtmt ]; then
        >&2 echo "ERROR: delete \`${coregit}/lib/irrlichtmt\`, or cross compilation will fail"
        exit 1
    fi
	rsync -av --exclude=$(basename $builddir) $coregit/ $CORE_NAME
}
